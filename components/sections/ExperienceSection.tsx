    ...
    <section id="experience" className="relative py-20 px-6 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto w-full">
        ...
        <div className="relative max-w-4xl mx-auto animate-fade-in delay-200">
          ...
          <div className="absolute left-6 top-6 bottom-6 w-px bg-pink-200 hidden md:block" />

          {displayed.length > 0 ? (
            <div className="space-y-8">
              {displayed.map((exp, index) => (
                <div key={exp.id ?? index} className="relative flex flex-col md:flex-row gap-6 md:pl-16">
                  {/* Timeline dot (desktop) */}
                  <div className="hidden md:flex absolute left-3.5 top-8 w-6 h-6 rounded-full bg-pink-200 border-4 border-white shadow-md items-center justify-center">
                    <FontAwesomeIcon icon={exp.type === 'pkl' ? faClipboardList : faSchool} className="w-3.5 h-3.5 text-pink-700" />
                  </div>

                  {/* Card */}
                  <div className="card flex-1 rounded-xs border-2 border-pink-300 bg-white/95 p-6 md:p-8 hover:shadow-xl hover:-translate-y-1 transition-all duration-200">
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                      <div>
                        <h3 className="text-lg md:text-xl font-bold text-pink-700">{exp.title}</h3>
                        <p className="text-pink-400 font-semibold text-sm mt-1">{exp.institution}</p>
                      </div>
                      {exp.period && (
                        <span className="inline-flex items-center gap-1.5 text-xs md:text-sm font-medium text-pink-700 bg-pink-100 px-3 py-1.5 rounded-xs border border-pink-300">
                    ...
                    {exp.description && (
                      <p className="text-pink-400 text-sm md:text-base leading-relaxed mb-4">{exp.description}</p>
                    )}

                    {exp.responsibilities.length > 0 && (
                      <ul className="space-y-2 mt-2">
                        {exp.responsibilities.map((r, i) => (
                          <li key={i} className="flex items-start gap-3 text-sm text-pink-700">
                            <span className="mt-0.5 shrink-0 w-6 h-6 rounded-full bg-pink-100 flex items-center justify-center">
                              <FontAwesomeIcon icon={faCheck} className="w-3 h-3 text-pink-400" />
                            </span>
